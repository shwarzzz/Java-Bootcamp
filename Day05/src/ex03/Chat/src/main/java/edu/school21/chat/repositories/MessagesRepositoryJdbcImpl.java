...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) {
        if (message == null || message.getId() == null) {
            throw new IllegalArgumentException("Message entity/id can't be null");
        }
        if (!findById(message.getId()).isPresent()) {
            throw new EntityDoesNotExistException("Message with id = "
                    + message.getId() + " does not exist");
        }
        try (Connection connection = getNewConnection()) {
            PreparedStatement statement = connection
                    .prepareStatement("UPDATE message " +
                            "SET author_id = ?, chatroom_id = ?, " +
                            "text = ?, date = ? " +
                            "WHERE id = ?");
            validateMessageSubEntity(message);
            if (!isUserEntityExist(connection, message.getAuthor().getId())) {
            ...
            setStatementParameters(message, statement);
    ...
    private void setStatementParameters(Message message,
